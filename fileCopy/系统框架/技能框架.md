...
在实体中控制buff的添加，通过唯一的接口来对buffcomponent进行操作，后面比较好维护。

## Buff Component

```lua
-- @Author: Jax
-- @Date:   2020-05-07 12:08:14

local LuaClass = LuaClass
local super = LuaClass.EcsComponent
---@class BuffComponent:EcsComponent
local BuffComponent = class("BuffComponent", super)

---@param buffs Buff[]
function BuffComponent:set(buffs)
    self.value = buffs
end

---@param buff Buff
function BuffComponent:addBuff(buff)
    table.insert(self.value, buff)
end

---@param buff Buff
function BuffComponent:removeBuff(buff)
    table.removebyvalue(self.value, buff)
end

---@return Buff
function BuffComponent:getBuffById(buffId)
    for _, buff in ipairs(self.value) do
        if buff._buffTempVO.ID == buffId then
            return buff
        end
    end
end
...
---@return Buff[]
function BuffComponent:getBuffsByLabel(buffLabel)
    local buffs = {}
    for _, buff in ipairs(self.value) do
        if isValidValue(buff._buffTempVO.BuffLabel) and table.indexof(buff._buffTempVO.BuffLabel, buffLabel) > 0 then
...
function BuffComponent:getAllBuffLabel(result)
    for _, buff in ipairs(self.value) do
        if isValidValue(buff._buffTempVO.BuffLabel) then
            for _, label in ipairs(buff._buffTempVO.BuffLabel) do
                result[label] = label
            end
        end
    end
...
---@return Buff[]
function BuffComponent:getBuffsByIds(buffIds)
    local buffs = {}
    local tempDict = table.create()
    for _, buffId in ipairs(buffIds) do
        tempDict[buffId] = buffId
    end
    for _, buff in ipairs(self.value) do
        if tempDict[buff._buffTempVO.ID] then
            table.insert(buffs, buff)
        end
    end
    table.release(tempDict)

    return buffs
end

function BuffComponent:getAllBuffIds()
    local buffIds = table.createAutoRelease()
    for _, buff in ipairs(self.value) do
        table.insert(buffIds, buff._buffTempVO.ID)
    end
    return buffIds
end
...
实体身上的buff都存在这个组件当中，里面放了操作这个组件的方法（建议放到外面去）。或者通过一个公共类来进行操作，而不是放到组件当中。

## Buff

```lua
-- @Author: Jax
-- @Date:   2020-05-07 15:42:33

local LuaClass = LuaClass
local super = nil
---@class Buff
local Buff = class("Buff", super)

---@param buffId number
---@param casterId string
---@param targetId string
---@param world EcsWorld
function Buff:ctor(buffId, casterId, targetId, world)
    --printJow("Buff:setCasterAndTarget", isValid(casterId), isValid(targetId))
    self.casterId = casterId
    self.targetId = targetId
    self.buffId = buffId
    self.world = world
    self._buffTempVO = LuaClass.BuffDatatable:getByID(buffId)
    if not self._buffTempVO then
        error("BuffId is error: " .. buffId)
    end
    -- 当前这种buff存在的数量, 一个数量上的buff对应一个duration
    self.count = 1
    self.duration = self._buffTempVO.Duration
    if isValidValue(self._buffTempVO.CountReduceCondition) then
        self.countReduceCondition = LuaClass.TriggerConditionGroup(self._buffTempVO.CountReduceCondition)
    end
    if isValidValue(self._buffTempVO.DurationReduceCondition) then
        self.durationReduceCondition = LuaClass.TriggerConditionGroup(self._buffTempVO.DurationReduceCondition)
    end
    ---@type BaseBuffMethod
    self.buffMethod = self:getBuffMethod()
end

---@param caster EcsEntity
---@param target EcsEntity
---buff第一次添加执行
function Buff:onAdd(caster, target)
    self:execute(caster, target)
    self.effectGoIds = table.create()
    for _, effectId in ipairs(self._buffTempVO.BuffEffect) do
        local effectGoId = target._owner:createEffect(target.id, effectId, false)
        table.insert(self.effectGoIds, effectGoId)
    end
end
...
---@param target EcsEntity
---buff状态发生改变执行
function Buff:execute(caster, target)
    self.isLock = true
    if not self.isEffecting then
        --if self.buffId == 10821 then
        --    printJow("Buff", "3006211")
        --end
        self.isEffecting = true
        self.buffMethod.caster = caster
        self.buffMethod.target = target
        --target:addTriggerEvent(LuaClass.FightEnum.TriggerType.AddBuff, self.buffId, self._buffTempVO.BuffLabel)
        --caster:addTriggerEvent(LuaClass.FightEnum.TriggerType.GiveBuff, self.buffId, self._buffTempVO.BuffLabel)
        self.buffMethod:onAdd(caster, target)
        self.buffMethod.caster = nil
        self.buffMethod.target = nil
    end
    if self.countAdd and self.countAdd > 0 then
        self.buffMethod.caster = caster
        self.buffMethod.target = target
        local count = self.countAdd
        self.countAdd = 0
        --target:addTriggerEvent(LuaClass.FightEnum.TriggerType.AddBuff, self.buffId, self._buffTempVO.BuffLabel)
        --caster:addTriggerEvent(LuaClass.FightEnum.TriggerType.GiveBuff, self.buffId, self._buffTempVO.BuffLabel)
        self.buffMethod:onCountAdd(caster, target, count)
        self.buffMethod.caster = nil
        self.buffMethod.target = nil
    end
    self.isLock = false
    ----持续时长
    if self.duration <= 0 or self.count <= 0 then
        self:finish()
    end
end

function Buff:dealReduceEvent(caster, target, event)
    if self.countReduceCondition and self.countReduceCondition:check(target, event) then
        self.count = self.count - 1
        self.buffMethod:onCountReduce(caster, target, 1)
    end
    if self.durationReduceCondition and self.durationReduceCondition:check(target, event) then
...
---@return BaseBuffMethod
function Buff:getBuffMethod()
    local buffMethodString = self._buffTempVO.BuffMethod
    ---@type BaseBuffMethod
    local effectBuff
    --- buff执行特效函数
    local buffMethodStr = string.gsub(buffMethodString, "%)", "")
    local buffMethodStrArr = string.split(buffMethodStr, "(")
    local name = buffMethodStrArr[1] .. "BuffMethod"
    self.buffMethodName = name
    local params = string.splitNumber(buffMethodStrArr[2], ",")
    self.buffMethodParams = params
    if LuaClassList[string.lower(name)] then
        effectBuff = LuaClass[name](self, self.world)
        effectBuff.buffId = self._buffTempVO.ID
        effectBuff.paramStr = buffMethodString
        effectBuff:preSetParams(params)
        effectBuff:setParams(params)
    else
        error("BuffMethod not find " .. name .. "   ID:" .. self._buffTempVO.ID)
    end
    return effectBuff
end

--有一个相同类型的buff添加了
function Buff:accumulate()
    if self.count < self._buffTempVO.MaxCount then
        self.countAdd = 1
        self.count = self.count + self.countAdd
    end
    self.duration = self._buffTempVO.Duration
end
...
function Buff:refresh()
    if not self.isFinish then
        local caster = self.world:getEntity(self.casterId)
        local target = self.world:getEntity(self.targetId)
        self.buffMethod:onCountChange(caster, target)
    end
end

---@return string[]
function Buff:getBuffLabel()
    return self._buffTempVO.BuffLabel
end
...
function Buff:remove(caster, target)
    ...
    if self.buffMethod then
        self.buffMethod.caster = caster
        self.buffMethod.target = target
        self.buffMethod:onRemove(caster, target, self.count)
        self.buffMethod.caster = nil
        self.buffMethod.target = nil
    end
    if self.effectGoIds then
        local len = #self.effectGoIds
        for i = 1, len do
            local effect = self.world:getEntity(self.effectGoIds[i])
            if effect then
                --effect:dispose()
                effect:set(LuaClass.DieReasonComponent)
                effect:set(LuaClass.FrameClockComponent, 10, 1)
            end
            self.effectGoIds[i] = nil
        end
        table.release(self.effectGoIds)
    end
end
...
对于buff，生效的方式主要是通过BuffMethod来实现的，buff是一个可持续过程，可以累积，可以更新，移除的时候是否需要移除效果。这些都是设计上的细节，需要去分类处理。
...
local LuaClass = LuaClass
local super = nil
---@class BaseBuffMethod
---@field caster EcsEntity
---@field target EcsEntity
local BaseBuffMethod = class("BaseBuffMethod", super)

---@param buff Buff
---@param world EcsWorld
function BaseBuffMethod:ctor(buff, world)
    self.buff = buff
    self.world = world
    self.silently = false
end

function BaseBuffMethod:preSetParams(params)
    self._params = params
    self._paramIndex = 0
end

function BaseBuffMethod:setParams(params)

end

function BaseBuffMethod:nextParams()
    self._paramIndex = self._paramIndex + 1
    return self._params[self._paramIndex]
end

--- 第一次添加
function BaseBuffMethod:onAdd(caster, target)
    self.silently = false
    self:effect(caster, target)
end

--- 真正的移除
function BaseBuffMethod:onRemove(caster, target)
    self.silently = false
    self:remove(caster, target)
end

function BaseBuffMethod:onCountAdd(caster, target)
    self:onCountChange(caster, target)
end

function BaseBuffMethod:onCountReduce(caster, target)
    self:onCountChange(caster, target)
end

function BaseBuffMethod:onCountChange(caster, target)
    self.silently = true
    self:remove(caster, target)
    self:effect(caster, target)
end
...
--- 每次作用
function BaseBuffMethod:effect(caster, target)
end

--- 移除作用
function BaseBuffMethod:remove(caster, target)
end

return BaseBuffMethod
```

这个类是游戏中所有buff method的基类，通过继承这个基类来实现具体的buff method。
...
-- @Des:    打断实体中可以被打断的技能
local LuaClass = LuaClass
local super = LuaClass.BaseBuffMethod
---@class InterruptSkillBuffMethod:BaseBuffMethod
local InterruptSkillBuffMethod = class("InterruptSkillBuffMethod", super)

---@param caster EcsEntity
---@param target EcsEntity
function InterruptSkillBuffMethod:effect(caster, target)
    local skillList = target.skill.value
    for _, skill in ipairs(skillList) do
        if skill.skillTemplateVO.Interrupt == 1 then
            --if skill.skillTemplateVO.ID == 30062 then
            --    printJow("InterruptSkillBuffMethod30062")
            --end
            skill._isActive = false
            skill.isFinish = true
        end
...
return InterruptSkillBuffMethod
...
local super = nil
---@class BuffSystem
local BuffSystem = class("BuffSystem", super)

---@param world EcsWorld
function BuffSystem:ctor(world)
    self.world = world
    self.buffFilter = world:getFilter({ LuaClass.BuffComponent })
end

function BuffSystem:execute()
    ---@param e EcsEntity
    for _, e in ipairs(self.buffFilter.entities) do
        ---@type Buff[]
        local copyBuffList = table.create()
        table.insertto(copyBuffList, e.buff.value)
        for _, buff in ipairs(copyBuffList) do
            local caster = self.world:getEntity(buff.casterId)
            local target = self.world:getEntity(buff.targetId)
            buff:execute(caster, target)
        end
        table.release(copyBuffList)
    end
end
