...
它可以帮助您提高游戏性能，适用于优化对象的反复创建和销毁。
 ...
 NodePool 需要实例化之后才能使用，每种不同的节点对象池需要一个不同的对象池实例，这里的种类对应于游戏中的节点设计，一个 prefab 相当于一个种类的节点。

在创建缓冲池时，可以传入一个包含 unuse, reuse 函数的组件类型用于节点的回收和复用逻辑。

##### 属性（properties）

- [`poolHandlerComp`](https://docs.cocos.com/creator/api/zh/classes/NodePool.html#poolhandlercomp) `Function|String` 缓冲池处理组件，用于节点的回收和复用逻辑，这个属性可以是组件类名或组件的构造函数。

##### 方法

- [`constructor`](https://docs.cocos.com/creator/api/zh/classes/NodePool.html#constructor) 使用构造函数来创建一个节点专用的对象池，您可以传递一个组件类型或名称，用于处理节点回收和复用时的事件逻辑。
- [`size`](https://docs.cocos.com/creator/api/zh/classes/NodePool.html#size) 获取当前缓冲池的可用对象数量
- [`clear`](https://docs.cocos.com/creator/api/zh/classes/NodePool.html#clear) 销毁对象池中缓存的所有节点
- [`put`](https://docs.cocos.com/creator/api/zh/classes/NodePool.html#put) 向缓冲池中存入一个不再需要的节点对象。
- [`get`](https://docs.cocos.com/creator/api/zh/classes/NodePool.html#get) 获取对象池中的对象，如果对象池没有可用对象，则返回空。
