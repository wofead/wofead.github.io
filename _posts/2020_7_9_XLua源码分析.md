...
我们在Lua中，经常会使用一些不是Lua层设置和赋值的全局变量，这些变量，要么是自己通过GLobal获取之后进行调用的，要么是在LuaEnv中的init_lua string进行赋值的。像我们常用的CS，cast， typeof，xlua相关的函数，这里还有一点，就是xlua的有些函数是通过另一种途径设置的，在ObjectTranslator中，OpenLib添加方法。
